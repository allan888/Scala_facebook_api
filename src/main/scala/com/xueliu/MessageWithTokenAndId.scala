...
object MessageWithTokenAndIdJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit val messagewithtokenandidFormat = jsonFormat6(MessageWithTokenAndId)
}

case class MessageWithTokenAndId(message: String, typ: String, photoData: String, albumId: Long, access_token: String, uid: Long) {
    ...
    ret1 match {
      case IdAndName(got_id, got_name) => {
        if (got_id == uid) {
          ...
          val future2 = contentActorSelection ? PostNoId(uid, got_name, typ, Calendar.getInstance.getTime.toString, message, photoData, album_id)
